...
def detect() -> PlatformCapabilities:
    ...
    if sys.platform == "win32":
        return PlatformCapabilities(
            os_name="windows", display_server="windows",
            has_global_hotkeys=True, has_global_mouse_events=True,
            caveats=[],
        )

    if sys.platform == "darwin":
        return PlatformCapabilities(
            os_name="macos", display_server="macos",
            has_global_hotkeys=True, has_global_mouse_events=True,
            caveats=[
    ...
    if sys.platform.startswith("linux"):
        ...
        if session == "wayland" or (not session and wayland_display):
            return PlatformCapabilities(
                os_name="linux", display_server="wayland",
        ...
        if session == "x11" or os.environ.get("DISPLAY"):
            return PlatformCapabilities(
                os_name="linux", display_server="x11",
                has_global_hotkeys=True, has_global_mouse_events=True,
                caveats=[],
            )
        return PlatformCapabilities(
            os_name="linux", display_server="unknown",
            has_global_hotkeys=False, has_global_mouse_events=False,
            caveats=["unknown_display_server"],
        )

    return PlatformCapabilities(
        os_name=sys.platform, display_server="unknown",
        has_global_hotkeys=False, has_global_mouse_events=False,
        caveats=["unknown_os"],
    )
...
def log_summary(caps: PlatformCapabilities) -> None:
    ...
    logger.info(
        ...
        caps.os_name, caps.display_server,
        caps.has_global_hotkeys, caps.has_global_mouse_events,
